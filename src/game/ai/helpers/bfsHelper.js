...
export function bfsToTarget(world, startX, startY, goalX, goalY, avoidDanger = true, entity = null) {
  ...
  while (queue.length > 0) {
    ...
    for (const dir of ALL_DIRS) {
      ...
      if (avoidDanger) {
        if (!isSafe(world, nx, ny, entity) && !(nx === goalX && ny === goalY)) continue
      } else if (!isWalkable(world, nx, ny, entity)) {
        continue
      }
...
export function bfsToSafeTile(world, startX, startY, entity = null) {
  const queue = [{ x: startX, y: startY, firstDir: null }]
  const visited = new Set([`${startX},${startY}`])
  ...
  while (queue.length > 0) {
    ...
    for (const dir of ALL_DIRS) {
      ...
      if (!isWalkable(world, nx, ny, entity)) continue

      const nextFirstDir = firstDir ?? dir

      if (isSafe(world, nx, ny, entity)) {
...
export function bfsLeafNodes(world, startX, startY, minDist = 1, maxDist = Infinity, entity = null) {
